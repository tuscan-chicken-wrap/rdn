...
class RelationalNaiveBayes(LocalModel):
    ...
    def predict_proba(self, data, rel_update_only=False):
        ...
        if not rel_update_only:
            ...
            un_features = features[data.mask.Unlabeled.values.nonzero()]
        ...
        # Relational IID Predictions
            ...
            if(self.neighbor_flag == 1):
                ...
                for node in data.mask.Unlabeled.nonzero()[0]:
                    neighbors = edge_matrix[node,data.mask.Labeled.nonzero()[0]].nonzero()[0] #Array containing all the neighboring indexes
                    temp.append(np.sum(data.features.values[data.mask.Labeled.nonzero()[0]][neighbors], axis=0).dot(self.feature_log_prob_y_given_neighbor_x_.values.T) + (len(neighbors)) * self.class_log_prior_.values)
                temp = np.array(temp)
    ...
    def fit(self, data, rel_update_only=False):
        if not rel_update_only:
            ...
            for ycl in data.labels.Y.columns:
                ...
                self.feature_log_prob_x_.loc[ycl, :] = np.log(np.minimum(features[idx, :].mean(axis=0)+1e-16,1+1e-16))
            ...
            if(self.neighbor_flag == 1):
                ...
                for ycl in data.labels.Y.columns:
                    ...
                    for node in data.mask.Labeled.nonzero()[0]:
                        ...
                        neighbors = edge_matrix[node,data.mask.Labeled.nonzero()[0]].nonzero()[0] #Array containing all the neighboring indexes
                        neighbors_idx = np.nan_to_num(data.labels.Y[ycl][data.mask.Labeled.nonzero()[0]])[neighbors].nonzero()[0]  #This is all neighbor indexes that have Y = ycl (0 or 1)
                        ...
                        if(neighbors_idx.size != 0):
                            currentMeanCalc = features[neighbors][neighbors_idx,:].mean(axis=0)
                            #currentMeanCalc+=1e-20
...
#             for ycl in data.labels.Y.columns:
#                 idx = np.nan_to_num(data.labels.Y[ycl].values * data.mask.Labeled.values).nonzero()[0]
#                 # print(np.minimum(features[idx, :].mean(axis=0)+1e-323,1+1e-323))
